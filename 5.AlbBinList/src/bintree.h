...
void bintree<T,N>::BFS(Nodo n){
	...
	while (!q.codaVuota()){
		//esamina q.top
		if(!sx_empty(q.leggiCoda())) q.inCoda(sx(q.leggiCoda()));
		if(!dx_empty(q.leggiCoda())) q.inCoda(dx(q.leggiCoda()));
		q.fuoriCoda();
	}
}
...
int bintree<T,N>::_width(Nodo n, int w){
	...
	while (!q.codaVuota()){
		...
		int j=0;
		while(j<i){
			Nodo top=q.leggiCoda();
			if (!sx_empty(top)) q.inCoda(sx(top));
			if (!dx_empty(top)) q.inCoda(dx(top));
			q.fuoriCoda();
			j++;
		}
	}
...
int bintree<T,N>::maxSumSubtree(const Nodo n)const{
	...
	if(!sx_empty(n)) q.inCoda(sx(n));
	if(!dx_empty(n)) q.inCoda(dx(n));
