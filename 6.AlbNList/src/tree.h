...
public:
	...
	const Linked_list<TNode<T> *>& childs(TNode<T> *n) const;
...
template<typename T>
const Linked_list<TNode<T> *>& Tree<T>::childs(TNode<T> *n) const
{
	return n->childs;
}
...
void Tree<T>::BST() const
{
	...
	while(!queue.empty())
	{
		...
		const Linked_list<TNode<T> *> &ch = childs(node);

		List_node<TNode<T> *> *tmp = ch.begin();
...
TNode<T>* Tree<T>::_copy(TNode<T> *n, TNode<T> *p)
{
	...
	const Linked_list<TNode<T> *> &ch = childs(n);
...
size_t Tree<T>::_width(TNode<T> *n, size_t w) const
{
	...
	while (!queue.empty())
	{
		...
		while(j < i)
		{
			const Linked_list<TNode<T> *> &ch = childs(queue.front());
...
void Tree<T>::_print(std::ostream &os, TNode<T> *n)
{
	...
	const Linked_list<TNode<T> *> &ch = n->childs;
